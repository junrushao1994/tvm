...
#endif
  TVM_NO_INLINE ~LogFatal() TVM_THROW_EXCEPTION { GetEntry().Finalize(); }
#ifdef _MSC_VER
#pragma disagnostic pop
 ...
 private:
  struct Entry {
    ...
    dmlc::Error Finalize() { throw InternalError(file_, lineno_, stream_.str()); }
  ...
  TVM_NO_INLINE Entry& GetEntry() { return entry_; }
  Entry entry_;
#else
  TVM_NO_INLINE Entry& GetEntry();
#endif
};
